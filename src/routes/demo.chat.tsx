...
const useAiAgent = () => {
   ...
   */
  React.useEffect(() => {
    const offChunk = api.aiAgent.on.chunk(({ answer }) => {
      setMessages((prev) => {
        setIsChunking(true)
    ...
    const offDone = api.aiAgent.on.done(({ answer }) => {
      setIsChunking(false)
      setIsLoading(false)
      setMessages((prev) => [
        ...prev,
    ...
    const offError = api.aiAgent.on.error(({ error }) => {
      setIsChunking(false)
      setIsLoading(false)
      setMessages((prev) => [
        ...prev,
  ...
  React.useEffect(() => {
    ...
    api.aiAgent.getHistory().then((history) => {
